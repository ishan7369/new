...
ADMIN_USERS = [5695538116,846159981]  # Replace with actual user IDs of admins
...
def is_user_admin(user_id, chat_id):
    return user_id in ADMIN_USERS or bot.get_chat_member(chat_id, user_id).status in ['administrator', 'creator']
...
def add_admin(message):
    ...
    if not is_user_admin(user_id, chat_id):
        bot.send_message(chat_id, "*You are not authorized to add admins.*", parse_mode='Markdown')
        return
    
    cmd_parts = message.text.split()
    
    if len(cmd_parts) != 2:
        bot.send_message(chat_id, "*Invalid command format. Use /add_admin <user_id>*", parse_mode='Markdown')
        return
    
    new_admin_id = int(cmd_parts[1])
    
    if new_admin_id in ADMIN_USERS:
        bot.send_message(chat_id, "*User is already an admin.*", parse_mode='Markdown')
    else:
        ADMIN_USERS.append(new_admin_id)
        bot.send_message(chat_id, f"*User {new_admin_id} added as admin.*", parse_mode='Markdown')

if __name__ == '__main__':
    update_proxy()
    Thread(target=loop.run_until_complete, args=(start_asyncio_thread(),)).start()
